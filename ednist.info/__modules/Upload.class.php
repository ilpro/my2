...
class Upload
{
  ...
  public static function createImageFullname($image, $name, $width = '', $wm = false)
  {
    global $ini;
    $im = new Imagick($image);
    ...
    if ($width > 0) $im->cropThumbnailImage($width, $height); // width, height
    $im->setImageFormat('jpg');

    $iWidth = $im->getImageWidth(); // width of thumb img
    $iHeight = $im->getImageHeight(); // height of thumb img
    if ($wm) {
  ...
  public static function cropImg($id, $c, $name)
  {
    ...
    if (is_array($c)) {
      if ($im->cropImage($c['w'], $c['h'], $c['x'], $c['y'])) {
        ...
        self::createImageFullname($folder . $name, "images/$id/big/400.jpg", 400);
        self::createImageFullname($folder . $name, "images/$id/big/600.jpg", 600);
        self::createImageFullname($folder . $name, "images/$id/big/800.jpg", 800);
      }
      if ($im2->cropImage($c['w'], $c['h'], $c['x'], $c['y'])) {
        $im2->setImageFormat('jpg');
        $im2->writeImage($folder2 . '800.jpg');
        $im2->destroy();
        self::createImageFullname($folder2 . '800.jpg', "images/$id/main/" . '800.jpg', 800);
      }
  ...
  public static function cropBrandImg($id, $c, $name)
  {
    ...
    if (is_array($c)) {
      ...
      if ($im2->cropImage($c['w'], $c['h'], $c['x'], $c['y'])) {
        $im2->setImageFormat('jpg');
        $im2->writeImage($folder2 . '400.jpg');
        $im2->destroy();
        self::createImageFullname($folder2 . '400.jpg', "brand/$id/main/" . '400.jpg', 400);
      }
      if ($im3->cropImage($c['w'], $c['h'], $c['x'], $c['y'])) {
        $im3->setImageFormat('jpg');
        $im3->writeImage($folder2 . '600.jpg');
        $im3->destroy();
        self::createImageFullname($folder2 . '600.jpg', "brand/$id/main/" . '600.jpg', 600);
      }
      if ($im4->cropImage($c['w'], $c['h'], $c['x'], $c['y'])) {
        $im4->setImageFormat('jpg');
        $im4->writeImage($folder2 . '800.jpg');
        $im4->destroy();
        self::createImageFullname($folder2 . '800.jpg', "brand/$id/main/" . '800.jpg', 800);
      }
      ...
      if ($im5->cropImage($c['w'], $c['h'], $c['x'], $c['y'])) {
        ...
        $im5->writeImage($folder3 . $name);
        $im5->destroy();
        self::createImageFullname($folder3 . $name, "brand/$id/gallery/" . $name, 800, false);
        return true;
      }
  ...
  public static function cropThemesImg($id, $c, $name)
  {
    ...
    if (is_array($c)) {
      if ($im->cropImage($c['w'], $c['h'], $c['x'], $c['y'])) {
        ...
        self::createImageFullname($folder . $name, "themes/$id/big/400.jpg", 400);
        self::createImageFullname($folder . $name, "themes/$id/big/600.jpg", 600);
        self::createImageFullname($folder . $name, "themes/$id/big/800.jpg", 800);
      }
      if ($im4->cropImage($c['w'], $c['h'], $c['x'], $c['y'])) {
        $im4->setImageFormat('jpg');
        $im4->writeImage($folder2 . '800.jpg');
        $im4->destroy();
        self::createImageFullname($folder2 . '800.jpg', "themes/$id/main/" . '800.jpg', 800);
      }
