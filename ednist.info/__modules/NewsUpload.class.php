...
class NewsUpload extends Upload
{
    ...
    if ($imgfiles) { // Загрузка фото из папки
      ...
      foreach ($imgfiles['error'] as $key => $error) {
        if ($error == UPLOAD_ERR_OK) { //print_r($key);
          // проверяем размер, что бы не грузили мелкие фото
          $im = new Imagick($imgfiles['tmp_name'][$key]);
          $imageprops = $im->getImageGeometry();
          if ( $imageprops['width'] < 800) continue;
          ...
          self::createImage($imgfiles['tmp_name'][$key], $targetpath . '/' . $newsId . "/big/400", 'jpg', 400);
          self::createImage($imgfiles['tmp_name'][$key], $targetpath . '/' . $newsId . "/big/600", 'jpg', 600);
          self::createImage($imgfiles['tmp_name'][$key], $targetpath . '/' . $newsId . "/big/800", 'jpg', 800);
          self::createImage($imgfiles['tmp_name'][$key], $targetpath . '/' . $newsId . "/big/" . $name, 'jpg', 800);
    ...
    if ($imgfiles) { // Загрузка фото из папки
      ...
      foreach ($imgfiles['error'] as $key => $error) {
        ...
        if ($error == UPLOAD_ERR_OK) { //print_r($key);
          ...
          self::createImage($imgfiles['tmp_name'][$key], $targetpath . '/' . $brandId . "/gallery/" . $name, 'jpg', 800, false);
    ...
    if ($imgfiles) { // Загрузка фото из папки
      ...
      foreach ($imgfiles['error'] as $key => $error)
        ...
        if ($error == UPLOAD_ERR_OK) { //print_r($key);
          ...
          self::createImage($imgfiles['tmp_name'][$key], $targetpath . '/' . $authorId . "/big/600", 'jpg', '600');
          self::createImage($imgfiles['tmp_name'][$key], $targetpath . '/' . $authorId . "/big/400", 'jpg', '400');
          self::createImage($imgfiles['tmp_name'][$key], $targetpath . '/' . $authorId . "/big/800", 'jpg', '800');
  ...
  public static function linkNewsImgs($link, $newsId, $returnType = 'html')
  {
    ...
    if (strlen($link) > 0) {
      ...
      if ($data == 1) {
        ...
        self::createImage($doclink, 'images/' . $newsId . "/big/400", 'jpg', '400');
        self::createImage($doclink, 'images/' . $newsId . "/big/600", 'jpg', '600');
        self::createImage($doclink, 'images/' . $newsId . "/big/800", 'jpg', '800');
  ...
  public static function linkBrandImgs($link, $brandId, $returnType = 'html')
  {
    ...
    if (strlen($link) > 0) {
      ...
      if ($data == 1) {
        ...
        self::createImage($doclink, 'brand/' . $brandId . "/gallery/" . $name, 'jpg', 800, false);
        self::createImage($doclink, 'brand/' . $brandId . "/big/400", 'jpg', 400);
        self::createImage($doclink, 'brand/' . $brandId . "/big/600", 'jpg', 600);
        self::createImage($doclink, 'brand/' . $brandId . "/big/800", 'jpg', 800);
  ...
  public static function linkThemesImgs($link, $themesId, $returnType = 'html')
  {
    ...
    if (strlen($link) > 0) {
      ...
      if ($data == 1) {
        ...
        self::createImage($doclink, 'themes/' . $themesId . "/big/400", 'jpg', '400');
        self::createImage($doclink, 'themes/' . $themesId . "/big/600", 'jpg', '600');
        self::createImage($doclink, 'themes/' . $themesId . "/big/800", 'jpg', '800');
  ...
  public static function linkAuthorImgs($link, $authorId, $returnType = 'html')
  {
    ...
    if (strlen($link) > 0) {
      ...
      if ($data == 1) {
        ...
        self::createImage($doclink, 'author/' . $authorId . "/big/400", 'jpg', '400');
        self::createImage($doclink, 'author/' . $authorId . "/big/600", 'jpg', '600');
        self::createImage($doclink, 'author/' . $authorId . "/big/800", 'jpg', '800');
  ...
  public static function mainNewsImgs($imgId)
  {
    ...
    self::createImage($ini['path.media'] . 'images/' . $result['newsId'] . "/big/" . $result['imgName'], 'images/' . $result['newsId'] . "/main/800", 'jpg', 800, false);
    return $res;
  }
  ...
  public static function mainBrandsImgs($imgId)
  {
    ...
    self::createImage($ini['path.media'] . 'brand/' . $result['brandId'] . "/raw/" . $result['imgName'], 'brand/' . $result['brandId'] . "/main/800", 'jpg', 800);
    self::createImage($ini['path.media'] . 'brand/' . $result['brandId'] . "/raw/" . $result['imgName'], 'brand/' . $result['brandId'] . "/main/600", 'jpg', 800);
  ...
  public static function mainThemesImgs($imgId)
  {
    ...
    self::createImage($ini['path.media'] . 'themes/' . $result['themesId'] . "/raw/" . $result['imgName'], 'themes/' . $result['themesId'] . "/main/800", 'jpg', 800);
    self::createImage($ini['path.media'] . 'themes/' . $result['themesId'] . "/raw/" . $result['imgName'], 'themes/' . $result['themesId'] . "/main/400", 'jpg', 400, false);
    return $res;
  }
  ...
  public static function delNewsImgs($imgId)
  {
    ...
    if (unlink($ini['path.media'] . "images/" . $result['newsId'] . "/gallery/" . $result['imgName'])) {
      ...
      unlink($ini['path.media'] . "images/" . $result['newsId'] . "/big/400.jpg");
      unlink($ini['path.media'] . "images/" . $result['newsId'] . "/big/600.jpg");
      unlink($ini['path.media'] . "images/" . $result['newsId'] . "/big/800.jpg");
  ...
  public static function delBrandImgs($imgId)
  {
    ...
    fclose($ini['path.media'] . "brand/" . $result['brandId'] . "/gallery/" . $result['imgName']);
  ...
  public static function delThemesImgs($imgId)
  {
    ...
    if (unlink($ini['path.media'] . "themes/" . $result['themesId'] . "/gallery/" . $result['imgName'])) {
      ...
      unlink($ini['path.media'] . "themes/" . $result['themesId'] . "/big/400.jpg");
      unlink($ini['path.media'] . "themes/" . $result['themesId'] . "/big/600.jpg");
      unlink($ini['path.media'] . "themes/" . $result['themesId'] . "/big/800.jpg");
