...
if(isset($_POST['action']) and $_POST['action']=='get_more') {
    ...
    $limit = 8;
    $newsList = $news->getNews(" WHERE `newsStatus`=4 AND `newsExclusive`=1 ORDER BY `newsTimePublic` DESC LIMIT ".$limit_from.','.$limit );
    $more = $news->getNews( '  WHERE `newsStatus`=4 AND `newsExclusive`=1 ORDER BY `newsTimePublic` DESC LIMIT  '.($limit_from+$limit).',1' )!=false ? true : false ;
...
$page_data['news'] = $news->getNews(" WHERE `newsStatus`=4 AND `newsExclusive`=1 ORDER BY `newsTimePublic` DESC LIMIT 8" );
