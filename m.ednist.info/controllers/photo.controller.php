...
if(isset($_POST['action']) and $_POST['action']=='get_more') {
    ...
    $limit = 9;
    $newsList = $news->getNews( ' WHERE `newsIsGallery`=1 AND `newsStatus`=4 ORDER BY `newsTimePublic` DESC LIMIT '.$limit_from.','.$limit );
    $more = $news->getNews( '  WHERE `newsIsGallery`=1 AND `newsStatus`=4 ORDER BY `newsTimePublic` DESC LIMIT  '.($limit_from+$limit).',1' )!=false ? true : false ;
...
$page_data['news_bottom'] = $news->getNews(" WHERE `newsIsGallery`=1 AND `newsStatus`=4 ORDER BY `newsTimePublic` DESC LIMIT 9" );
...
$sliders['important'] = $news->getNews(' WHERE `newsMain`=1 AND `newsType`=0 AND `newsStatus`=4 ORDER BY `newsTimePublic` DESC  LIMIT 8');//$page_data['check_articles_count'] = count( $news->getNews(' WHERE `newsType`=1 ORDER BY `newsTimePublic` DESC LIMIT 6,6 ') ) > 0 ? true : false ;