...
if(isset($_POST['action']) and $_POST['action']=='get_more') {
    ...
    $limit = 8;
    $newsList = $news->getNews( ' WHERE (`newsTimePublic` > (now() - interval 1 day)) AND `newsStatus`=4 ORDER BY `countVisits` DESC LIMIT '.$limit_from.','.$limit );
    $more = $news->getNews( '  WHERE (`newsTimePublic` > (now() - interval 1 day)) AND `newsStatus`=4 ORDER BY `countVisits` DESC LIMIT  '.($limit_from+$limit).',1' )!=false ? true : false ;
    include "../views/block/exclusiveBlock.php.view.php";

   // $banner = NewsLineBanner::getRandBanner($news, $ini, 'popular');
   // $res = $banner . $html;
...
$page_data['news'] = $news->getNews(' WHERE (`newsTimePublic` > (now() - interval 1 day)) AND `newsStatus`=4 ORDER BY `countVisits` DESC LIMIT 8 ');
